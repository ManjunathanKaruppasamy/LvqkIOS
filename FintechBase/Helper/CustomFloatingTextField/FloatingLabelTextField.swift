...
//
//  Created by SENTHIL KUMAR on 12/07/22.
//
...
public enum ResignResponderFlag {
    case `default`
    case customDateTransaction
}
...
open class FloatingLabelTextField: UITextField { // swiftlint:disable:this type_body_length
    ...
    /// A `ResignResponderFlag` value that determines where the error message will be displayed.
    open var firstResponderFlag: ResignResponderFlag = .default
    
    open override func canPerformAction(_ action: Selector, withSender sender: Any?) -> Bool {
        UIMenuController.shared.isMenuVisible = false
        // do not display the menu
        if self.firstResponderFlag == .customDateTransaction {
    ...
    @IBInspectable dynamic open var errorColor: UIColor = .redErrorColor {
        didSet {
            updateColors()
        }
    }
...
//            titleLabel.textColor = titleErrorColor ?? errorColor
    ...
    fileprivate func updateTextColor() {
        if !isEnabled {
            ...
            super.textColor = cachedTextColor // textErrorColor ?? errorColor
        } else {
    ...
    fileprivate func updateTitleLabel(_ animated: Bool = false) {
        ...
        if errorMessagePlacement == .default {
            if hasErrorMessage {
                titleText = titleOrPlaceholder() // titleFormatter(errorMessage!)
            } else {
            ...
            if hasErrorMessage {
                errorText = titleFormatter(errorMessage ?? String())
            }
            if editingOrSelected {
                titleText = selectedTitleOrTitlePlaceholder()
                if titleText == nil {
...
//        }
    ...
    */
    open func titleLabelRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        if editing {
            return CGRect(x: 0, y: -30, width: bounds.size.width, height: titleHeight())
        }
