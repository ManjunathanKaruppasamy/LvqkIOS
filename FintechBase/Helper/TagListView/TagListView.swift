...
@objc public protocol TagListViewDelegate: AnyObject {
...
open class TagListView: UIView {
    ...
    @objc open dynamic var textFont: UIFont = .setCustomFont(name: .regular, size: .x10) {
        didSet {
            defer { rearrangeViews() }
            tagViews.forEach {
                $0.textFont = textFont
...
//            tagBackgroundView.layer.shadowRadius = shadowRadius
            ...
            switch alignment {
            case .left: currentRowView.frame.origin.x = 0
            case .center: currentRowView.frame.origin.x = (frameWidth - (currentRowWidth - marginX)) / 2
            case .right: currentRowView.frame.origin.x = frameWidth - (currentRowWidth - marginX)
            default:
                break
            }
        ...
        rows = currentRow
        self.delegate?.viewHeight(currentRowView.frame.origin.y + currentRowView.frame.size.height)
        invalidateIntrinsicContentSize()
    }
    ...
    open func addTags(_ titles: [String]) -> [TagView] {
         addTagViews(titles.map(createNewTagView))
    }
