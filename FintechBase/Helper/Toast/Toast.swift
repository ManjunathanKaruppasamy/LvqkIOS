 ...
 */
// swiftlint: disable implicit_getter force_unwrapping missing_docs
...
public enum ToastPosition {
    ...
    fileprivate func centerPoint(forToast toast: UIView, inSuperview superview: UIView) -> CGPoint {
        ...
        case .bottom:
            return CGPoint(x: superview.bounds.size.width / 2.0, y: (superview.bounds.size.height - (toast.frame.size.height / 3.0)) - bottomPadding)
        case .betweenBottomAndCenter:
            return CGPoint(x: superview.bounds.size.width / 2.0, y: superview.bounds.size.height / 1.25)
        }
    }
}
